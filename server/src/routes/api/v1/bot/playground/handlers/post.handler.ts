...
import { KurobaseHybridRetrival } from "../../../../../../utils/hybrid";
...
export const chatRequestHandler = async (
  ...
  try {
    ...
    if (bot.use_hybrid_search) {
      retriever = new KurobaseHybridRetrival(embeddingModel, {
        botId: bot.id,
        sourceId: null,
        callbacks: [
          {
      ...
      const vectorstore = await KurobaseVectorStore.fromExistingIndex(
        embeddingModel,
        {
          botId: bot.id,
          sourceId: null,
    ...
    const modelinfo = await prisma.kurobaseModels.findFirst({
      where: {
        model_id: bot.model,
        hide: false,
        deleted: false,
