...
export const createIntergationHandler = async (
  ...
  switch (request.body.provider) {
    case "telegram":
      const process_name_tg = `kurobase-telegram-${id}`;
    ...
    case "discord":
      const process_name_dc = `kurobase-discord-${id}`;
    ...
    case "whatsapp":
      const process_name_wa = `kurobase-whatsapp-${id}`;
    ...
    case "slack":
      const process_name_sl = `kurobase-slack-${id}`;
